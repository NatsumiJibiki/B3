...
public abstract class GameBase extends TimerTask {
	...
	protected static int WORLD_X_LENGTH = 1024;
	protected static int WORLD_Y_LENGTH = 768;
	...
	public GameBase() {
		...
		Y_INSETS = insets.top;
		frame.setSize(X_LENGTH + insets.left + insets.right, Y_LENGTH + insets.top + insets.bottom);
	...
	// 座標変換用関数
	// ======================================
	private int translate(int y, int yLength) {
	...
	private int getScrollX(int x, int px) {
		return x - getShiftX(px);
	}

	private int getScrollY(int y, int py) {
		return y - getShiftY(py);
	}

	private int getWorldX(int x, int px) {
		return x + getShiftX(px);
	}

	private int getWorldY(int y, int py) {
		return y + getShiftY(py);
	}
	...
	protected class Mouse implements MouseListener, MouseMotionListener {
		...
		public int getX(int px) {
			return getWorldX(x, px);
		}

		public int getY(int py) {
			return getWorldY(translate(y, 0), py);
		}
	...
	public class CG {
		...
		// 曲線
		protected void drawArc(int x, int y, int xLength, int yLength, int startAngle, int arcAngle) {
			g.drawArc(x, translate(y, yLength), xLength, yLength, startAngle, arcAngle);
		}

		protected void fillArc(int x, int y, int xLength, int yLength, int startAngle, int arcAngle) {
			g.fillArc(x, translate(y, yLength), xLength, yLength, startAngle, arcAngle);
		}
		...
		// （角の丸い）長方形
		protected void drawRoundRect(int x, int y, int xLength, int yLength, int arcxLength, int arcyLength) {
			g.drawRoundRect(x, translate(y, yLength), xLength, yLength, arcxLength, arcyLength);
		}

		protected void fillRoundRect(int x, int y, int xLength, int yLength, int arcxLength, int arcyLength) {
			g.fillRoundRect(x, translate(y, yLength), xLength, yLength, arcxLength, arcyLength);
		}
		...
		protected void drawStringCenter(int x, int y, int xLength, String str) {
			g.drawString(str, x + (xLength - getStringWidth(str)) / 2, translate(y, 0));
		}

		protected void drawStringRight(int x, int y, int xLength, String str) {
			g.drawString(str, x + (xLength - getStringWidth(str)), translate(y, 0));
		}
		...
		protected void drawImage(int x, int y, int xLength, int yLength, BufferedImage img) {
		...
		protected void drawImage(int x, int y, BufferedImage img, int px, int py) {
			...
			drawImage(sx + 3, sy, img);
		}

		protected void drawImage(int x, int y, int xLength, int yLength, BufferedImage img, int px, int py) {
			int sx = getScrollX(x, px);
			int sy = getScrollY(y, py);
